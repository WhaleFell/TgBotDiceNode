...
var server = app.listen(3888, function () {
    setInterval(function () {
        ...
        if (date.getSeconds() == 45 && minutes % conf.gameTime == 0 && conf.isfptx) {//封盘倒计时提醒
            fengpantixing();
        }

        if (date.getSeconds() >= 55 && minutes % conf.gameTime == 0 && !isfengpan) { //封盘提醒
            isfengpan = true;
            getAllBet();
        }

        if (date.getSeconds() >= 58 && minutes % conf.gameTime == 0 && !iskaijiang) { //开奖
            iskaijiang = true;
            setResult();
        }
...
var allkeyword = ["/ckye", "/zjtz", "/ckls", "/lqfs", "撤回", "全部撤回", "历史", "上分", "下分", "大", "小", "单", "双", "豹子", "对子", "顺子", "杀"]
/*监听新的文字消息*/
bot.on('message', (msg) => {
...
function main(msg) {
    ...
    } else if (msg.text.search("/ckye") != -1) {
